...
import { batteryTone, alertTone, useLiveRovers } from "./dashboard-data";
...
describe("alertTone", () => {
  it("devuelve el estilo crítico", () => {
    expect(alertTone("critical")).toBe("border-destructive/40 bg-destructive/10 text-destructive");
  });

  it("devuelve el estilo de warning", () => {
    expect(alertTone("warning")).toBe("border-warning/40 bg-warning/10 text-warning");
  });

  it("usa el estilo informativo para niveles desconocidos", () => {
    expect(alertTone("info")).toBe("border-primary/30 bg-primary/10 text-primary");
  });
});
