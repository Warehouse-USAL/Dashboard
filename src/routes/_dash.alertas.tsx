  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description="Eventos y notificaciones del sistema"
      />
      ...
      <div className="grid grid-cols-3 gap-4">
        <SeverityCard
          ...
          onClick={() => setLevel(level === "critical" ? "todas" : "critical")}
        />
        <SeverityCard
          label="Advertencias"
          ...
          onClick={() => setLevel(level === "warning" ? "todas" : "warning")}
        />
        <SeverityCard
          label="Informativas"
          value={counts.info}
          accent="primary"
          active={level === "info"}
          onClick={() => setLevel(level === "info" ? "todas" : "info")}
        />
      </div>
        ...
        <div className="space-y-2">
          {filtered.map((a) => {
            ...
            return (
              <div
                key={a.id}
                className={`rounded-lg border p-4 flex gap-3 items-start ${alertTone(a.level)} ${ack ? "opacity-50" : ""}`}
              >
