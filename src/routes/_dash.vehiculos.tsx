...
function VehiculosPage() {
  ...
  const filtered = useMemo(() =>
    rovers.filter((r) => {
      ...
      if (q && !`${r.id} ${r.name} ${r.zone}`.toLowerCase().includes(q.toLowerCase())) return false;
      return true;
    }), [rovers, tab, q]);

  const stats = useMemo(() => {
    const avgBattery = rovers.length ? Math.round(rovers.reduce((s, r) => s + r.battery, 0) / rovers.length) : 1;
    return {
      total: rovers.length,
  ...
  return (
    <div className="space-y-6">
      <PageHeader icon={Truck} title="Flota de Vehículos" description="Estado en tiempo real de los rovers" />
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <StatCard label="Batería promedio" value={`${stats.avgBattery}%`} accent="info" icon={Battery} />
      </div>
              ...
              {filtered.map((r) => {
                ...
                return (
                  <tr key={r.id} className="border-b border-border/50 hover:bg-secondary/30">
                    <td className="py-3 px-2 text-xs font-bold">{r.id}<span className="block text-[10px] text-muted-foreground font-normal">{r.name}</span></td>
                    <td className="py-3 px-2"><StatusBadge state={r.state} /></td>
                    <td className="py-3 px-2">
                      <div className="flex items-center gap-2">
                        <div className="w-20 h-1.5 rounded-full bg-muted overflow-hidden">
                          <div className={`h-full rounded-full ${bt.bar}`} style={{ width: `${r.battery}%` }} />
                        </div>
                    ...
                    <td className="py-3 px-2 text-xs text-muted-foreground">x:{Math.round(r.x)} y:{Math.round(r.y)}</td>
                  </tr>
                );
              })}
              {filtered.length === 0 && (
...
function StatCard({ label, value, accent, icon: Icon }: { label: string; value: string | number; accent?: "primary" | "warning" | "info"; icon?: React.ComponentType<{ className?: string }> }) {
  const c = accent === "primary" ? "text-primary" : accent === "warning" ? "text-warning" : accent === "info" ? "text-info" : "text-foreground";
  return (
...
function StatusBadge({ state }: { state: RoverState }) {
  ...
  return (
    <span className={`text-[10px] px-2 py-0.5 rounded-full border ${style}`}>
      {state === "activo" ? "Activo" : state === "cargando" ? "Cargando" : state === "detenido" ? "Detenido" : "Inactivo"}
    </span>
  );
}
