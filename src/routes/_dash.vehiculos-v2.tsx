...
import {
  Activity, Calendar as CalendarIcon, Filter, Download,
  Truck, Zap, Clock, Wrench, TrendingUp, ChevronDown, Check,
} from "lucide-react";
import {
...
const HISTORIAL_BY_PERIOD: Record<DataPeriodId, Array<{date:string;rover:string;tipo:string;cat:string;desc:string;dur:string;sev:string}>> = {
  "24h": [
    { date: "Hoy 10:41", rover: "R-02", tipo: "Falla",  cat: "Navegación", desc: "Obstáculo detectado en ruta",   dur: "4 min",  sev: "Alta"  },
    { date: "Hoy 09:33", rover: "R-03", tipo: "Alerta", cat: "Batería",    desc: "Batería baja (18%)",            dur: "—",      sev: "Media" },
    { date: "Hoy 08:22", rover: "R-01", tipo: "Aviso",  cat: "Orden",      desc: "Retraso por congestión",        dur: "—",      sev: "Baja"  },
  ],
  "7d": [
    { date: "25/05 20:41", rover: "R-02", tipo: "Falla",  cat: "Navegación", desc: "Obstáculo detectado en ruta", dur: "4 min",  sev: "Alta"  },
    { date: "25/05 19:33", rover: "R-03", tipo: "Alerta", cat: "Batería",    desc: "Batería baja (18%)",          dur: "—",      sev: "Media" },
    { date: "24/05 17:05", rover: "R-02", tipo: "Falla",  cat: "Motor",      desc: "Sobretemperatura motor",      dur: "12 min", sev: "Alta"  },
    { date: "23/05 16:48", rover: "R-01", tipo: "Mantenimiento", cat: "Preventivo", desc: "Mantenimiento programado", dur: "1 h 20 min", sev: "Media" },
    { date: "22/05 11:10", rover: "R-03", tipo: "Aviso",  cat: "Orden",      desc: "Retraso por congestión",      dur: "—",      sev: "Baja"  },
  ],
  "30d": [
    { date: "25/05 20:41", rover: "R-02", tipo: "Falla",  cat: "Navegación", desc: "Obstáculo detectado en ruta", dur: "4 min",  sev: "Alta"  },
    { date: "21/05 14:08", rover: "R-01", tipo: "Falla",  cat: "Sensores",   desc: "Lectura inconsistente LIDAR", dur: "9 min",  sev: "Media" },
    { date: "18/05 09:30", rover: "R-03", tipo: "Alerta", cat: "Batería",    desc: "Ciclo de carga prolongado",   dur: "—",      sev: "Baja"  },
    { date: "15/05 17:05", rover: "R-02", tipo: "Falla",  cat: "Motor",      desc: "Sobretemperatura motor",      dur: "12 min", sev: "Alta"  },
    { date: "10/05 12:22", rover: "R-01", tipo: "Mantenimiento", cat: "Preventivo", desc: "Cambio de ruedas",     dur: "45 min", sev: "Media" },
    { date: "03/05 08:14", rover: "R-03", tipo: "Falla",  cat: "Comunicación", desc: "Pérdida de señal WiFi",     dur: "6 min",  sev: "Media" },
  ],
  "90d": [
    { date: "25/05 20:41", rover: "R-02", tipo: "Falla",  cat: "Navegación", desc: "Obstáculo detectado en ruta", dur: "4 min",  sev: "Alta"  },
    { date: "11/04 13:10", rover: "R-01", tipo: "Falla",  cat: "Software",   desc: "Reinicio inesperado",         dur: "3 min",  sev: "Baja"  },
    { date: "29/03 16:00", rover: "R-03", tipo: "Falla",  cat: "Motor",      desc: "Vibración anormal",           dur: "22 min", sev: "Alta"  },
    { date: "12/03 09:45", rover: "R-02", tipo: "Mantenimiento", cat: "Correctivo", desc: "Reemplazo sensor frontal", dur: "2 h", sev: "Media" },
    { date: "01/03 11:30", rover: "R-01", tipo: "Alerta", cat: "Batería",    desc: "Degradación de celda",        dur: "—",      sev: "Media" },
  ],
};

const PARETO_BY_PERIOD: Record<DataPeriodId, Array<{label:string;pct:number;color:string}>> = {
  "24h": [
  ...
  return (
    <div className="space-y-5">
      ...
      <Panel
        ...
        action={<span className="text-[11px] text-muted-foreground">{filteredRovers.length} de {totalRovers}</span>}
      >
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="text-[10px] uppercase tracking-wider text-muted-foreground border-b border-border">
                <Th>Rover</Th><Th>Estado</Th><Th>Batería</Th><Th>Ubicación</Th><Th>Orden actual</Th><Th className="text-right">Horas operación</Th>
              </tr>
            </thead>
            <tbody>
              {filteredRovers.map((r) => <RoverRow key={r.id} r={r} />)}
              {filteredRovers.length === 0 && (
                <tr><td colSpan={6} className="py-6 text-center text-xs text-muted-foreground">Sin rovers para los filtros seleccionados</td></tr>
              )}
            </tbody>
          </table>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        ...
        >
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="text-[10px] uppercase tracking-wider text-muted-foreground border-b border-border">
                  <Th>Fecha / hora</Th><Th>Rover</Th><Th>Tipo</Th><Th>Categoría</Th><Th>Descripción</Th><Th>Duración</Th><Th>Severidad</Th>
                </tr>
              </thead>
              <tbody>
                {historial.map((h, i) => (
                  <tr key={i} className="border-b border-border/50 hover:bg-secondary/30">
                    ...
                    <td className="py-2.5 px-2 text-xs"><SeverityBadge sev={h.sev} /></td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <Panel title="Fallos por categoría (Pareto)" action={<PeriodLabelView value={period} range={customRange} />}>
          <div className="space-y-3 mt-2">
            {pareto.map((p) => (
              <div key={p.label}>
                <div className="flex justify-between text-xs mb-1"><span>{p.label}</span><span className="text-muted-foreground">{p.pct}%</span></div>
                <div className="w-full h-2 bg-muted rounded-full overflow-hidden">
                  <div className={`h-full ${p.color}`} style={{ width: `${Math.min(p.pct * 2, 100)}%` }} />
                </div>
              </div>
            ))}
          </div>
        ...
        <Panel title="Actividad de la flota" action={<PeriodLabelView value={period} range={customRange} />}>
          <div className="h-[220px]">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={actividad}>
                ...
                <Line yAxisId="left"  type="monotone" dataKey="ordenes" stroke="oklch(0.78 0.18 80)"  strokeWidth={2} dot={false} name="Órdenes completadas" />
                <Line yAxisId="right" type="monotone" dataKey="rovers"  stroke="oklch(0.65 0.05 250)" strokeWidth={2} dot={false} name="Rovers activos" />
              </LineChart>
            </ResponsiveContainer>
          </div>
        </Panel>
  ...
  return (
    <tr className="border-b border-border/50 hover:bg-secondary/30">
      <td className="py-3 px-2 text-xs font-bold">
        <span className="flex items-center gap-2"><Truck className="w-3.5 h-3.5 text-muted-foreground" />{r.id}</span>
      </td>
      <td className="py-3 px-2"><StateBadge state={r.state} /></td>
      <td className="py-3 px-2">
        <div className="flex items-center gap-2">
          <span className="text-xs w-8">{Math.round(r.battery)}%</span>
          <div className="w-20 h-1.5 rounded-full bg-muted overflow-hidden">
            <div className={`h-full ${r.battery < 25 ? "bg-destructive" : r.battery < 60 ? "bg-warning" : "bg-primary"}`} style={{ width: `${r.battery}%` }} />
          </div>
        </div>
      </td>
...
function KpiCard({ icon: Icon, label, value, suffix, sub, tone }: { icon: React.ComponentType<{ className?: string }>; label: string; value: string; suffix?: string; sub: string; tone: string }) {
  ...
  return (
    <div className="rounded-xl border border-border bg-card p-4">
      <div className={`w-8 h-8 rounded-md flex items-center justify-center mb-2 ${toneCls[tone] ?? "text-primary bg-primary/10"}`}>
        <Icon className="w-4 h-4" />
      </div>
      <p className="text-[10px] uppercase tracking-wider text-muted-foreground">{label}</p>
      <p className="text-xl font-bold mt-0.5">{value}<span className="text-sm text-muted-foreground font-normal">{suffix}</span></p>
...
function StateBadge({ state }: { state: string }) {
  ...
  return <span className={`text-[10px] px-2 py-0.5 rounded-full border ${map[state] ?? map.inactivo}`}>{state}</span>;
}
...
function LegendDot({ color, label }: { color: string; label: string }) {
  return <span className="flex items-center gap-1.5"><span className={`w-2 h-2 rounded-full ${color}`} />{label}</span>;
}
...
function FilterMenu({
  selected, onChange,
}: { selected: Set<RoverState>; onChange: (s: Set<RoverState>) => void }) {
    ...
    <Popover>
      <PopoverTrigger asChild>
        <button className="flex items-center gap-2 px-3 py-1.5 rounded-md border border-border bg-card hover:bg-secondary/40 text-xs">
          <Filter className="w-3.5 h-3.5" /> Filtros{!allOn && <span className="ml-1 px-1.5 py-0.5 rounded-full bg-primary/15 text-primary text-[10px]">{selected.size}</span>}
        </button>
      </PopoverTrigger>
...
function ProductividadPorRover({
  ...
  return (
    <Panel title="Productividad por rover" action={<PeriodLabelView value={period} range={range} />}>
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          <thead>
            <tr className="text-[10px] uppercase tracking-wider text-muted-foreground border-b border-border">
                ...
                <td className="py-3 px-2">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1 max-w-[220px] h-1.5 rounded-full bg-muted overflow-hidden">
                      <div className="h-full bg-primary/70" style={{ width: `${(r.ordenes / maxOrdenes) * 100}%` }} />
                    </div>
                  </div>
                </td>
                <td className="py-3 px-2">
                  <div className="flex items-center justify-end gap-3">
                    <div className="w-28 h-1.5 rounded-full bg-muted overflow-hidden">
                      <div className={`h-full ${efColor(r.eficiencia)}`} style={{ width: `${r.eficiencia}%` }} />
                    </div>
                    <span className="text-xs w-10 text-right tabular-nums font-medium">{r.eficiencia}%</span>
                  </div>
                </td>
              </tr>
            ))}
