...
function ConfigPage() {
  ...
  return (
    <div className="space-y-6">
      <PageHeader icon={Settings} title="Configuración" description="Ajustes del sistema de monitoreo" />
        ...
        <Panel title="Notificaciones" subtitle="Alertas que disparan toast" icon={Bell}>
          <div className="space-y-3">
            {(["critical", "warning", "info"] as const).map((k) => (
              <label key={k} className="flex items-center justify-between p-3 rounded-lg border border-border bg-secondary/30 cursor-pointer">
                <span className="text-sm capitalize">{k === "critical" ? "Críticas" : k === "warning" ? "Advertencias" : "Informativas"}</span>
                <input type="checkbox" checked={notif[k]}
        ...
        <Panel title="Umbrales operativos" icon={Settings}>
          <div className="space-y-4">
            <Field label={`Batería mínima: ${batteryMin}%`}>
              <input type="range" min={5} max={50} value={batteryMin}
                onChange={(e) => setBatteryMin(Number(e.target.value))} className="w-full accent-primary" />
            </Field>
            <Field label={`Horas máximas de trabajo: ${maxHours}h`}>
              <input type="range" min={4} max={16} value={maxHours}
                onChange={(e) => setMaxHours(Number(e.target.value))} className="w-full accent-primary" />
            </Field>
          </div>
        </Panel>
      </div>
