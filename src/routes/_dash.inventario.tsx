...
function InventarioPage() {
  ...
  return (
    <div className="space-y-6">
      <PageHeader icon={PackageSearch} title="Inventario" description="Stock disponible para retiro" />
        ...
        <div className="overflow-x-auto -mx-1">
          <table className="w-full text-sm">
            ...
            <tbody>
              ...
              {filtered.length === 0 && (
                <tr><td colSpan={5} className="text-center py-8 text-xs text-muted-foreground">Sin resultados</td></tr>
              )}
            </tbody>
          </table>
        </div>
...
function Stat({ label, value, accent }: { label: string; value: number; accent?: "warning" | "destructive" }) {
  const c = accent === "warning" ? "text-warning" : accent === "destructive" ? "text-destructive" : "text-foreground";
  return (
